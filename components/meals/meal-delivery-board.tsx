...
export function MealDeliveryBoard() {
  ...
  const totalDeliveries = visibleRuns.reduce(
    (s, m) => s + m.participants.length,
    0,
  );
  const vehiclesAssigned = new Set(
    visibleRuns.filter((m) => m.vehicleId).map((m) => m.vehicleId),
  ).size;
  const driversAssigned = new Set(
    visibleRuns.filter((m) => m.driverId).map((m) => m.driverId),
  ).size;
  const mapVehicles = vehicles.filter((v) =>
    visibleRuns.some((m) => m.vehicleId === v.id),
  );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="grid grid-cols-2 gap-px border-b border-border bg-border sm:grid-cols-4">
        <Kpi
          icon={Truck}
          label={t("meal.activeruns")}
          value={String(visibleRuns.length)}
        />
        <Kpi
          icon={Boxes}
          label={t("meal.deliveries")}
        ...
        <Kpi
          ...
          value={String(driversAssigned)}
        />
      </div>

      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[380px_1fr] lg:grid-rows-1">
        {/* Run list */}
              ...
              {filteredMealRuns.map((m) => {
                ...
                return (
                  <button
                    key={m.id}
                    onClick={() =>
                      setSelectedId(isSelected ? null : String(m.id))
                    }
                    className={cn(
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-muted-foreground">
                        {center?.name} · {m.participants.length}{" "}
                        {t("meal.deliveries").toLowerCase()}
                      </p>
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        {formatMonthDayYear(m.fromDate)} ·{" "}
  ...
  return (
    <div className="absolute right-0 top-0 z-600 flex h-full w-full max-w-sm flex-col overflow-y-auto border-l border-border bg-card shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-px border-b border-border bg-border">
        ...
        <InfoCell
          icon={Clock}
          label={t("meal.departs")}
          value={formatTimeOfDay(run.departTime)}
        />
        <InfoCell
          icon={Boxes}
          label={t("meal.deliveries")}
        ...
        <InfoCell
          icon={MapPin}
          label={t("common.date")}
          value={formatMonthDayYear(run.fromDate)}
        />
      </div>

      <div className="border-b border-border px-4 py-3">
        <p className="text-xs text-muted-foreground">
          {t("board.driverlabel")} {driver?.name ?? t("common.unassigned")}
        </p>
      </div>
            ...
            {run.participants.map((p) => (
              <li key={p.id} className="relative">
                ...
                <p className="text-sm font-medium">
                  {findById(participants, p.participantId)?.name ??
                    p.participantId}
                </p>
              </li>
            ))}
  ...
  return (
    <div className="flex items-center gap-3 bg-card px-4 py-3">
      ...
      <div className="min-w-0">
        <p
          className={cn("text-lg font-semibold leading-none tabular-nums", cls)}
        >
          {value}
        </p>
