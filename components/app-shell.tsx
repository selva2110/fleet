  ...
  const NAV: NavSection[] = [
    {
      ...
      items: [
        ...
        {
          href: "/planner",
          label: "nav.routeplanner",
          icon: Waypoints,
        },
      ],
    },
    {
      title: "nav.clientmanagement",
      items: [
        {
          href: "/participants",
          label: "common.participants",
          icon: Users,
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          href: "/meal-delivery",
          label: "e.mealdelivery",
          icon: Utensils,
        },
        {
          href: "/catalog",
          label: "e.catalogtitle",
          icon: Pill,
        },
        {
          href: "/trips",
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active
                        ? "border-l-3 border-sidebar-ring bg-sidebar-ring/15 text-sidebar-ring ring-1 ring-sidebar-ring/20"
                  ...
                  return (
                    <div key={item.href} className="flex flex-col gap-1">
                      ...
                      {hasChildren && itemOpen ? (
                        <div className="ml-4 flex flex-col gap-1 border-border pl-3">
...
function LiveClock() {
  ...
  return (
    <span className="hidden whitespace-nowrap font-mono text-sm tabular-nums text-sidebar-foreground/70 sm:inline">
  ...
  useEffect(() => {
    const checkReminders = async () => {
      ...
      const dueReminders = events.flatMap((event) => {
        ...
        const hasAssignedVehicle = eventTrips.some((trip) =>
          Boolean(trip.vehicleId),
        );

        const hasAssignedDriver = eventTrips.some((trip) =>
        ...
        return (event.reminders ?? []).flatMap((reminder) => {
          ...
          if (reminder.sent || firedReminderKeysRef.current.has(key)) {
            return [];
          }
      ...
      try {
        await Promise.all(
          dueReminders.map(({ eventId, reminders }) =>
            markEventReminderSent(eventId, reminders),
          ),
        );
 ...
 * Once an event's response deadline passes, the plan
 * is generated and committed automatically with no
 * manual intervention — the dispatcher only manually
 * triggers a replan.
 */
  ...
  useEffect(() => {
    const autoPlan = async () => {
      ...
      const candidates = events.filter((event) => {
        if (plannedRef.current.has(event.id)) {
          return false;
        }
        ...
        if (!TripsUtils.isEventDispatchable(event, now)) {
          return false;
        }
...
export function NotificationCenter() {
  ...
  const { notifications, clearNotifications, dismissNotification } =
    useNotifications();
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-background">
      <ReminderMonitorRoot />
      <header className="z-30 shrink-0 border-b border-sidebar-border bg-sidebar text-sidebar-foreground shadow-sm">
        <div className="flex h-14 items-center gap-3 px-4">
          <Button
            size="icon"
            className={cn(
              sidebarOpen
                ? "bg-sidebar-ring text-sidebar"
                : "bg-transparent text-sidebar-ring",
            )}
            onClick={() => setSidebarOpen((v) => !v)}
            aria-label={
              sidebarOpen ? t("common.hidemenu") : t("common.showmenu")
            }
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        <aside
          className={cn(
            ...
            "fixed bottom-0 left-0 top-14 z-30 w-72 lg:static lg:top-0 lg:z-0",
            sidebarOpen
