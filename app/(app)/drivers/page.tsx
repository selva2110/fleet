...
export default function DriversPage() {
  ...
  const certified = drivers.filter(
    (d) => d.certifications.cprCert.enabled || d.certifications.cprCert.enabled,
  ).length;
  const activeFilterCount =
    (statuses.length ? 1 : 0) +
    (certs.length ? 1 : 0) +
    (assignment.length ? 1 : 0);
  ...
  const filtered = useMemo(() => {
    ...
    const list = drivers.filter((d) => {
      ...
      const matchCerts =
        certs.length === 0 ||
        certs.every(
          (c) => d.certifications[c as keyof Driver["certifications"]].enabled,
        );
    ...
    return list.sort((a, b) =>
      compareValues(
        a[dv.sortKey as keyof Driver],
        b[dv.sortKey as keyof Driver],
        dv.sortDir,
      ),
    );
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <RowActions
                                variant="menu"
                                onEdit={() => openEdit(d)}
                                onDelete={() => deleteDriver(d.id, d.name)}
                                onPto={() =>
                                  router.push(`/drivers/pto?driverId=${d.id}`)
                                }
                                deleteTitle={t("driver.delete")}
                                deleteMessage={t("driver.deletcnfrm").replace(
                                  "{{name}}",
                                  d.name,
                                )}
                              />
              ...
              <Card className="overflow-hidden py-0">
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {pg.pageItems.map((d) => {
                        const meta = DriversConfig.getDriverStatusMeta(
                          d.status,
                        );
                        const vehicle = d.assignedVehicleId
                          ? findById(vehicles, d.assignedVehicleId)
                          ...
                          <TableRow key={d.id}>
                            ...
                            <TableCell>
                              <div className="flex flex-wrap gap-1">
                                {d.certifications.cprCert.enabled ? (
                                  <Badge
                                    variant="secondary"
                                ...
                                {d.certifications.nemCert.enabled ? (
                                  <Badge
                                    variant="secondary"
                                    className="px-1 py-0 text-[10px]"
                                  >
