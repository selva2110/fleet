...
export default function ResponsesPage() {
  ...
  const scopedRows = useMemo<PartResponseRow[]>(() => {
    ...
    const hardcodedStatuses: Array<{
      deliveryStatus: SmsNotification['deliveryStatus']
      response: SmsNotification['response']
    }> = [
      { deliveryStatus: 'queued', response: null },
      { deliveryStatus: 'queued', response: null },
      { deliveryStatus: 'failed', response: null },
    ]

    let hardcodedIndex = 0
    const usedParticipantIds = new Set<string>()
    let painFailAssigned = false

    for (const row of rows) {
      if (hardcodedIndex < hardcodedStatuses.length && !usedParticipantIds.has(row.participant.id)) {
        usedParticipantIds.add(row.participant.id)
        const override = hardcodedStatuses[hardcodedIndex]
        const baseNotif = row.notif
        row.notif = baseNotif
          ? {
              ...baseNotif,
              deliveryStatus: override.deliveryStatus,
      ...
      if (row.event.name === 'Pain Self Management Program') {
        const failNames = new Set(['Marcus Bellamy'])
        const isFail = failNames.has(row.participant.name)
        row.notif = {
          ...row.notif,
          deliveryStatus: isFail ? 'failed' : 'sent',
          response: isFail ? null : 'attending_transport',
          responseBody: null,
          respondedAt: isFail ? null : PartResponseUtils.randomPastDate(5),
          updatedAt: new Date().toISOString(),
        }
      }
    }
