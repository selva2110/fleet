...
export default function ResponsesPage() {
  ...
  const { sendEventNotifications, assignTransport, processDueReminders } =
    useNotificationActions();
  ...
  const eventOptions = useMemo(() => {
    const opts = [...events]
      ...
      .map((e) => ({
        value: e.id,
        label: `${e.name} · ${formatMonthDayYear(e.date)}`,
      }));
  ...
  const notifIndex = useMemo(() => {
    ...
    for (const n of smsNotifications)
      map.set(`${n.eventId}:${n.participantId}`, n);
    return map;
  }, [smsNotifications]);
    ...
    for (const { notif } of scopedRows) {
      if (notif) {
        ...
        if (
          ...
          notif.deliveryStatus === "received"
        )
          s.delivered += 1;
        if (
          notif.deliveryStatus === "queued" ||
          notif.deliveryStatus === "sent"
        )
          s.pending += 1;
        if (
          notif.deliveryStatus === "failed" ||
          notif.deliveryStatus === "undelivered"
        )
          s.failed += 1;
      }
    ...
    return scopedRows
      ...
      .filter(({ notif }) => {
        ...
        if (deliveryFilter === "delivered")
          return d === "delivered" || d === "sent";
        if (deliveryFilter === "replied")
          return d === "received" || Boolean(notif.response);
        if (deliveryFilter === "pending") return d === "queued";
        if (deliveryFilter === "failed")
          return d === "failed" || d === "undelivered"
        return true;
      })
      .filter(({ participant }) => {
        if (!q) return true;
  ...
  console.log(scopedRows)

  // Counts per response filter, so the rail pills can show live badges.
  const responseOptions = useMemo(
    () => [
      {
        value: "all" as ResponseFilter,
        label: t("resp.allresponses"),
        count: stats.total,
      },
      {
        value: "attending_transport" as ResponseFilter,
        label: t("resp.needstransport"),
        count: stats.needTransport,
        dot: "bg-primary",
      },
      {
        value: "attending_self" as ResponseFilter,
        label: t("e.owntransport"),
        count: stats.attendingSelf,
        dot: "bg-success",
      },
      {
        value: "not_attending" as ResponseFilter,
        label: t("e.notattending"),
        count: stats.notAttending,
        dot: "bg-destructive",
      },
      {
        value: "none" as ResponseFilter,
        label: t("e.noresponse"),
  ...
  const deliveryOptions = useMemo(
    () => [
      ...
      {
        value: "delivered" as DeliveryFilter,
        label: t("common.delivered"),
        dot: "bg-success",
      },
      {
        value: "replied" as DeliveryFilter,
        label: t("resp.replied"),
        dot: "bg-primary",
      },
      {
        value: "pending" as DeliveryFilter,
        label: t("e.pending"),
        dot: "bg-warning",
      },
      {
        value: "failed" as DeliveryFilter,
        label: t("e.failed"),
        dot: "bg-destructive",
      },
    ],
  ...
  const singleEvent =
    eventFilter === "all"
      ? null
      : (events.find((e) => e.id === eventFilter) ?? null);
  ...
  function exportResponses() {
    ...
    const data = rows.map(({ participant, event, notif }) => [
      ...
      participant.phone,
      notif
        ? (PartResponseConfig.deliveryMeta[notif.deliveryStatus]?.label ??
          notif.deliveryStatus)
        : t("e.notsent"),
      notif?.response
        ? t(PartResponseConfig.RESPONSE_META[notif.response].label)
        : t("e.noresponse"),
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="min-w-0 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <StatCard
                label={t("notif.notifications")}
                value={stats.sent}
                icon={MessageSquare}
                hint={`${stats.total} ${t("resp.onroster")}`}
              />
              <StatCard
                label={t("common.delivered")}
                  ...
                  <TableBody>
                    {rows.map(({ key, participant, event, notif, idx }) => (
                      <TableRow key={key}>
                        ...
                        <TableCell>
                          {notif ? (
                            <StatusBadge
                              label={
                                t(
                                  PartResponseConfig.deliveryMeta[
                                    notif.deliveryStatus
                                  ]?.label,
                                ) ?? notif.deliveryStatus
                              }
                        ...
                        <TableCell>
                          <span className="inline-flex flex-col items-center gap-0.5 text-xs text-muted-foreground">
                            {notif?.response ? (
                              <StatusBadge
                                label={t(
                                  PartResponseConfig.RESPONSE_META[
                                    notif.response
                                  ].short,
                                )}
                                cls={
