...
import { PageHeader, ConstraintChips } from "@/components/common";
...
export default function ParticipantsPage() {
  ...
  const { participants, pagination, isLoading } = useParticipants({
    page: page,
    limit: pageSize,
    sortBy: dv.sortKey,
          ...
          filters={
            <FilterRail activeCount={activeFilterCount} onReset={resetFilters}>
              <FilterSection title={t("part.specialneeds")}>
                <CheckboxGroupFilter
                  options={ParticipantConfig.NEEDS_OPTIONS}
                  selected={needs}
                  onChange={setNeeds}
                />
              </FilterSection>
                      ...
                      <div className="flex flex-col gap-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <RowActions
                            ...
                            deleteMessage={t("part.deleteConfirm").replace(
                              "{{name}}",
                              p.name,
                            )}
                          />
                        ...
                        <div className="flex items-center gap-1 text-xs text-muted-foreground">
                          <Clock className="size-3" /> ≤ {p.maxTravelMinutes}{" "}
                          {t("common.min")}
                        </div>
                      </div>
                          ...
                          <TableRow key={p.id}>
                            ...
                            <TableCell>
                              <p className="flex items-center gap-1 text-sm text-muted-foreground">
                                ...
                                {p.maxTravelMinutes} {t("common.min")}
                              </p>
                            </TableCell>
                            <TableCell>
                              {p.companionNeeded ? (
