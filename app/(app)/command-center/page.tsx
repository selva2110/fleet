...
import { EventGroupListProps, FleetEvent } from "@/lib/events/types";
import { TripDetailProps } from "@/lib/trips/types";
...
export default function DispatchPage() {
  ...
  const tripsTabLoading = tripsLoading || eventsLoading;
  const feedTabLoading = eventLogLoading;
  ...
  const liveEventGroups = useMemo(() => {
    const groups = new Map<
      string,
      { event: FleetEvent | undefined; trips: typeof liveTrips }
    >();
    for (const t of liveTrips) {
      ...
      if (existing) existing.trips.push(t);
        ...
        groups.set(t.eventId, {
          event: findById(events, t.eventId),
          trips: [t],
        });
    }
  ...
  const selectedVehicle = selectedTrip
    ? findById(vehicles, selectedTrip.vehicleId)
    : undefined;
  ...
  useEffect(() => {
    ...
    if (deepLinkedEventId) {
      const eventTrips = liveTrips.filter(
        (t) => t.eventId === deepLinkedEventId,
      );
      setHighlightedTripIds(eventTrips.map((t) => t.id));
      return;
    }
  ...
  const displayedTrips = tab === "all-trips" ? trips : liveTrips;
  ...
  const mapParticipants = useMemo(() => {
    ...
    for (const t of displayedTrips) {
      if (!Array.isArray(t.stops)) continue;
      for (const s of t.stops) ids.add(s.participantId);
    }
    return participants.filter((p) => ids.has(p.id));
  }, [displayedTrips, participants]);

  const activeStatusVehicles = vehicles.filter((v) =>
    VehiclesConfig.ACTIVE_VEHICLE_STATUSES.includes(v.status),
  );
  ...
  const totalStops = liveTrips.reduce(
    (sum, t) => sum + (Array.isArray(t.stops) ? t.stops.length : 0),
    0,
  );
  const pickedStops = liveTrips.reduce(
    (sum, t) =>
      ...
      (Array.isArray(t.stops)
        ? t.stops.filter((s) => s.status === "picked-up").length
        : 0),
    0,
  );
  const pickupsRemaining = totalStops - pickedStops;
  const topLiveEventGroups = useMemo(
    () =>
      liveEventGroups
        .map(({ event, trips: groupTrips }) => ({
          event,
          trips: groupTrips,
          progress: Math.round(
            (groupTrips.reduce((s, t) => s + t.progress, 0) /
              groupTrips.length) *
              100,
          ),
        }))
  ...
  const utilization =
    vehicles.length > 0
      ? Math.round((activeStatusVehicles.length / vehicles.length) * 100)
      : 0;
  const nextArrival = liveTrips.map((t) => t.etaCenter).sort()[0];
  ...
  const trackingFeed = useMemo(
    () =>
      eventLog
        .filter(
          (e) => e.aggregateType === "trip" || e.aggregateType === "vehicle",
        )
        .slice(0, 60),
    [eventLog],
  );
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
        ...
        actions={
          <>
            <div className="hidden items-center gap-1.5 sm:flex">
              ...
              >
                <Waypoints className="size-4" /> {t("Route Planner")}
              </Button>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <Send className="size-4" /> {t("Event Responses")}
              </Button>
      ...
      <Tabs
        ...
        className="flex min-h-0 flex-1 flex-col gap-0 my-3"
      >
        <TabsList className="mx-4 my-1 w-fit">
          <TabsTrigger value="dispatch">
            <Bus className="size-3.5" /> {t("cc.livedispatch")}
            <CountBadge count={liveTrips.length} />
          </TabsTrigger>
          <TabsTrigger value="meals">
            <UtensilsCrossed className="size-3.5" /> {t("e.mealdelivery")}
            <CountBadge count={mealRunCount} />
          </TabsTrigger>
        </TabsList>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-3 px-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              icon={Bus}
              label={t("cc.activetrips")}
              value={liveTrips.length}
              tone={liveTrips.length > 0 ? "primary" : "default"}
            />
            <StatCard
              icon={Users}
              label={t("cc.onboard")}
              value={onboardCount}
              tone={onboardCount > 0 ? "primary" : "default"}
            />
            <StatCard
              icon={MapPin}
              label={t("cc.remainingpickups")}
              value={pickupsRemaining}
              hint={t("cc.completedhint")
                .replace("{{picked}}", String(pickedStops))
                .replace("{{total}}", String(totalStops))}
            />
            <StatCard
              icon={Clock}
              label={t("cc.nextarrival")}
              value={nextArrival ?? "—"}
            />
          </div>

          {/* Operational insights row */}
          <div className="mt-2 grid grid-cols-1 gap-4 px-4 sm:grid-cols-1 lg:grid-cols-3">
            <InsightCard
              title={t("cc.routeprogress")}
              tooltip={t("cc.routeprogresstooltip")}
              description={t("cc.routeprogressdesc")}
            >
              <div className="mt-3 flex flex-1 flex-col justify-center gap-3">
                {topLiveEventGroups.length === 0 ? (
                  <p className="text-xs text-muted-foreground">
                    {t("cc.noactivetrips")}
                  </p>
                ) : (
                  topLiveEventGroups.map(
                    ({ event, trips: groupTrips, progress }) => {
                      const eventId = event?.id ?? groupTrips[0].eventId;
                      return (
                        <div key={eventId}>
            ...
            <InsightCard
              title={t("cc.driverstatus")}
              tooltip={t("cc.driverstatustooltip")}
              description={t("cc.driverstatusdesc")}
            >
              <div className="mt-3 grid flex-1 grid-cols-3 items-center gap-2">
                <div className="text-center">
                  <div className="text-lg font-semibold">
                    {
                      drivers.filter((driver) =>
                        events.some((event) =>
                          DriverUtils.isDriverOnShift(
                            driver,
                            event.date,
                            event.startTime,
                          ),
            ...
            <InsightCard
              title={t("cc.vehicleutilization")}
              tooltip={t("cc.vehicleutiltooltip")}
              description={t("cc.vehicleutildesc")}
            >
              <div className="mt-3 flex flex-1 items-center justify-between">
                <div className="w-full mr-4">
                  <MiniBar value={utilization} />
                  <div className="mt-2 text-sm font-medium">
                    {t("cc.activetotal")
                      .replace(
                        "{{active}}",
                        String(activeStatusVehicles.length),
                      )
          ...
          <div className="flex min-h-0 flex-1 flex-col lg:flex-row gap-2 rounded-md p-4">
            <div className="flex min-h-0 flex-col bg-card rounded-t-md">
              <Tabs
                value={tab}
                onValueChange={(v) =>
                  setTab(v as "trips" | "feed" | "all-trips")
                }
                className="flex flex-col items-center justify-center max-h-100 gap-0"
              >
                <TabsList className="grid grid-cols-3 my-2">
                  <TabsTrigger value="all-trips" className={"truncate"}>
                    {" "}
                    {t("cc.alltrips")}
                    {trips.length ? <CountBadge count={trips.length} /> : null}
                  </TabsTrigger>
                ...
                >
                  {tripsTabLoading ? (
                    ...
                    <EventGroupList
                      groups={filteredLiveEventGroups}
                      isEmpty={liveEventGroups.length === 0}
                      searchValue={liveEventSearch}
                      onSearchChange={setLiveEventSearch}
                ...
                >
                  {feedTabLoading ? (
                    ...
                    <ScrollArea className="min-h-0 w-87.5">
                      <div className="border-t border-border">
                        <EventFeed
                          events={trackingFeed}
                          dense
                          emptyLabel={t("cc.waitingliveevents")}
                        />
                      </div>
                    </ScrollArea>
                  )}
                ...
                >
                  {tripsTabLoading ? (
                    ...
                    <EventGroupList
                      ...
                      isEmpty={eventGroups.length === 0}
                      searchValue={allEventSearch}
                      onSearchChange={setAllEventSearch}
                      expandedEventId={expandedEventId}
                      onToggleExpand={setExpandedEventId}
                      selectedTripId={selectedTripId}
                      onSelectTrip={setSelectedTripId}
...
function EventGroupList({
  ...
  return (
    <div className="flex min-h-0 flex-1 w-87.5 flex-col rounded-md">
      <div className="relative mx-3 m-2 shrink-0">
          ...
          <div className="divide-y divide-border border-t border-border">
            ...
            {groups.length === 0 ? (
              <p className="px-4 py-8 text-center text-xs text-muted-foreground">
                {isEmpty ? t("cc.noactivetrips") : t("cc.noeventsmatch")}
              </p>
            ) : null}
          </div>
        </ScrollArea>
...
function InsightCard({
  ...
  return (
    <div className="col-span-1 lg:col-span-1 h-full">
      <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card/90 shadow-sm p-4">
...
function TripDetail({
  ...
  const tripStartMs = trip.startedAt
    ? new Date(trip.startedAt).getTime()
    : Date.now();
  const stops = Array.isArray(trip.stops) ? trip.stops : [];
  const nextStop = stops.find(
    (s) => s.status === "pending" || s.status === "approaching",
  );
  const nextStopParticipant = nextStop
    ? findById(participants, nextStop.participantId)
    : undefined;
  const liveEtaMin = nextStop
    ? Math.max(
        1,
        estimateMinutes(haversineKm(trip.currentLocation, nextStop.location)),
      )
    : null;

  return (
    <div className="absolute right-0 top-0 z-45 overflow-y-auto flex h-full w-full max-w-xs flex-col border-l border-border bg-card shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-px border-b border-border bg-border">
        ...
        <InfoCell
          icon={Clock}
          label={t("cc.etatocenter")}
          value={trip.etaCenter}
        />
        <InfoCell
          icon={MapPin}
          label={t("meal.distance")}
          value={formatMiles(trip.distanceKm)}
        />
        <InfoCell
          icon={Bus}
          label={t("cc.duration")}
          value={`${trip.durationMinutes} ${t("common.min")}`}
        />
      </div>
      ...
      {nextStop && liveEtaMin != null ? (
        <div className="flex items-center gap-2 border-b border-border bg-primary/5 px-4 py-2.5">
          ...
          <p className="min-w-0 flex-1 truncate text-xs text-foreground">
            {t("common.next")}:{" "}
            <span className="font-medium">{nextStopParticipant?.name}</span>
            <span className="text-muted-foreground">
              {" "}
              — ~{liveEtaMin} {t("cc.minaway")}
            </span>
          </p>
          ...
          >
            <SelectTrigger className="h-9">
              <SelectValue placeholder={t("cc.selectdriver")}>
                {(value) =>
                  findById(drivers, String(value))?.name ?? t("cc.selectdriver")
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
              {availableDrivers.map((d) => {
                const onShift = event
                  ? DriverUtils.isDriverOnShift(d, event.date, event.startTime)
                  : true;
                return (
                  <SelectItem key={d.id} value={d.id}>
                    {d.name} · {d.rating.toFixed(1)}★
                    {onShift ? "" : ` ${t("cc.offshift")}`}
                  </SelectItem>
                );
              })}
            </SelectContent>
        ...
        <div className="p-4">
          ...
          <ol className="relative space-y-4 border-l border-dashed border-border pl-5">
            {stops.map((stop) => {
              ...
              return (
                <li key={stop.participantId} className="relative">
                  ...
                  >
                    {done ? (
                      <CircleDot className="size-2.5 text-white" />
                    ) : null}
                  </span>
                  <div className="flex items-center justify-between gap-2">
            ...
            <li className="relative">
              ...
              <p className="text-xs text-muted-foreground">
                {t("cc.destinationlabel")} · {center?.address}
              </p>
            </li>
          </ol>
        </div>
