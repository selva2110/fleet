...
import * as rolesApi from '@/lib/api/auth'
...
import { GroupForm } from "@/lib/catalog/groups";
...
export async function markEventReminderSent(
  eventId: string,
  reminders: FleetEvent["reminders"],
) {
  ...
  await emit({
    ...
    payload: {
      reminders: (reminders ?? []).filter((reminder) => reminder.sent).length,
    },
  });
}
...
export async function saveParticipantMedReport(
...
export async function deleteRole(id: number, name: string, actorRole = 'admin') {
  ...
  await emit({
    ...
    aggregateId: String(id),
    actorRole,
    summary: `Removed role ${name}`,
  })
}

export async function updateRole(id: number, data: {name:string, description:string, status:boolean}, actorRole = 'admin') {
  const res = await rolesApi.editRoleById(id, data)
  await emit({
...
export async function saveGroup(input: GroupForm, actorRole = "dispacther") {
  const isNew = !input.id;
  const user = isNew
    ? await mealsApi.createGroup(input)
    : await mealsApi.updateGroup(input);
  ...
  await emit({
    ...
    actorRole: actorRole,
    summary: `${isNew ? "Added" : "Updated"} user ${input.name}`,
    payload: { roleIds: input.name, status: input.id },
  });
  return user.id;
}

export async function deleteCatalogGroups(id: number, actorRole = "admin") {
  await mealsApi.deleteCatalogGroup(id);
  await emit({
    eventType: "user.deleted",
    aggregateType: "user",
    aggregateId: String(id),
